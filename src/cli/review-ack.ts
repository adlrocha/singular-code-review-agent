...
export async function acknowledgeReviewRequest(options: {
  ...
  try {
    ...
    return {
      shouldReview: true,
      message: `could not list trigger reactions; continuing review: ${getErrorMessage(error)}`
    }
  }
  ...
  if (reactions.some(reaction => reaction.content === "eyes" && reaction.user?.login === options.botLogin)) {
    ...
    return {
      shouldReview: false,
      message: `trigger comment already acknowledged by ${options.botLogin}; skipping review`
    }
  }
  ...
  try {
    ...
    return {
      shouldReview: true,
      message: `could not acknowledge trigger comment; continuing review: ${getErrorMessage(error)}`
    }
  }
